...
void incUpdate_paths(int node, int l, int r, int lq, int rq, int add) {
    ...
    segtree_paths[node] = max(segtree_paths[node * 2 + 1], segtree_paths[node * 2 + 2]); //nebitno
}

int getMax_paths(int l, int r, int lq, int rq, int node) {
    if (l > rq || lq > r) {
        return -INF;
    }
    apply_paths(node, l, r);
    ...
    int mid = (l + r) / 2;
    return max(getMax_paths(l, mid, lq, rq, 2 * node + 1),
           getMax_paths(mid + 1, r, lq, rq, 2 * node + 2));
}
...
int getMax(int l, int r, int lq, int rq, int node, int n) {
    if (l > rq || lq > r || lq > rq) {
        return -INF;
    }
    apply(node, l, r, n);
    ...
    int mid = (l + r) / 2;
    return max(getMax(l, mid, lq, rq, 2 * node + 1, n),
           getMax(mid + 1, r, lq, rq, 2 * node + 2, n));
}
...
int calc(int a, int b, int flag) {
    ...
    while (1) {
        int j = pos[a].first;
        if (j == pos[b].first) {
            mx = max(mx, getMax(0, paths[j].size() - 1, pos[a].second, pos[b].second - flag, 0, j) + getMax_paths(0, paths.size() - 1, path_pos[j], path_pos[j], 0));
            break;
        }
        mx = max(mx, getMax(0, paths[j].size() - 1, pos[a].second, paths[j].size() - 1, 0, j) + getMax_paths(0, paths.size() - 1, path_pos[j], path_pos[j], 0));
        a = parent[tail[a]];
    }
    return mx;
}
