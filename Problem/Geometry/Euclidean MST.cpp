...
int main() {
    ...
    if (t.size() == 0) {
        ...
        for (int i = 1; i < n; i++) {
            cout << id[{p[ord[i - 1]].x, p[ord[i - 1]].y}] << " " << id[{p[ord[i]].x, p[ord[i]].y}] << endl;
        }
        return 0;
    }
    ...
    for (int i = 0; i < t.size(); i += 3) {
        int a = id[{t[i].x, t[i].y}];
        int b = id[{t[i + 1].x, t[i + 1].y}];
        int c = id[{t[i + 2].x, t[i + 2].y}];
        edges.push_back(make_tuple(dist(t[i], t[i + 1]), a, b));
        edges.push_back(make_tuple(dist(t[i], t[i + 2]), a, c));
        edges.push_back(make_tuple(dist(t[i + 1], t[i + 2]), b, c));
    }
