//https://ocpc2026w.eolymp.space/en/compete/g6s5o5k4n93719vrc5sak0hi3k/problem/5
#include <bits/stdc++.h>
...
ll calc(int l, int r) {
    ...
    int mid = (l + r) / 2;
    ll ans = (calc(l, mid) + calc(mid + 1, r)) % M;
    ...
    for (int i = mid + 1; i <= r; i++) {
        b.push_back((ll)freq[i] * i % M);
    }
    vector<ll> c = convMod(a, b);
    for (int i = 0; i < c.size(); i++) {
        ans = (ans + c[i] * inv[i + l + mid + 1]) % M;
    }
    return ans;
