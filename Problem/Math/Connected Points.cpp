//https://dmoj.ca/problem/btoi07p5
...
struct chash {
    int operator()(pair<int, long long> x) const { return x.first* 100 + x.second; }
};

int ct = 0;

int ind(int n, int x, int y) {
    return n * x + y;
}

pair<int, int> mp(int n, int x, int y, int xx, int yy) {
    int i = ind(n, x, y), j = ind(n, xx, yy);
    return make_pair(min(i, j), max(i, j));
}

void calc(int n, int x, int y, int k, vector<vector<bool>> &b, gp_hash_table<pair<int, int>, int, chash>  &st) {
    if (x == 2 && !b[0][n - 1]) {
        return;
    }
    if (x == 1 && y >= 2 && !b[0][y - 2]) {
    ...
    if (y < n - 1 && b[2][n - 1] && !b[2][0] && !b[x][y + 1]) {
        return;
    }
    if (x == 2 && !b[0][y]) {
        return;
    ...
    for (int dx = -1; dx <= 1; dx++) {
        for (int dy = -1; dy <= 1; dy++) {
            if (x + dx >= 0 && x + dx < 3 && y + dy >= 0 && y + dy < n) {
                if (abs(dx) + abs(dy) == 2 && st.find(mp(n, x + dx, y, x, y + dy)) != st.end()) {
                    continue;
                }
                if (x + dx == 0 && y + dy == 0 && k == 3 * n - 1) {
                    ct++;
                    return;
                }
                if (y == -1 && !b[0][n - 1]) {
                ...
                b[x + dx][y + dy] = true;
                st[mp(n, x, y, x + dx, y + dy)] = 1;
                calc(n, x + dx, y + dy, k + 1, b, st);
                st.erase(mp(n, x, y, x + dx, y + dy));
                b[x + dx][y + dy] = false;
            }
        }
    }
...
int main() {
    ...
    for (int i = 10; i <= 9; i++) {
        int n = i;
        vector<vector<bool>> b(3, vector<bool>(n, false));
        gp_hash_table<pair<int, int>, int, chash> st;
        b[0][0] = true;
        calc(n, 0, 0, 0, b, st);
        ct = 0;
    }
    vector<long long> s{1, 8, 40, 192, 928, 4480, 21632, 104448};
    LinearRecurrence c(s, M, 0);
    int n;
    cin >> n;
    cout << c.solve(n - 2);
    return 0;
