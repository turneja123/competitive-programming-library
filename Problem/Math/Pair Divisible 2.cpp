...
int main() {
    ...
    while (t--) {
        cin >> n >> x >> y;
        //n = 897612484786617600, x = 1e18, y = 1e18;
        if (x > y) {
            swap(x, y);
        }
        factor(n);
        ...
        long long ans = 0;
        for (int k = 0; k < d; k++) {
            long long complement = n / masks[k].second, h = masks[k].first.second;
            long long b = (y / complement) % M;
            ans += a[h] % M * b % M;
    ...
    /*1
897612484786617600 1000000000000000000 1000000000000000000
122514222*/
/*1
776313500355993600 1000000000000000000 1000000000000000000
986319492
*/
    return 0;
}
