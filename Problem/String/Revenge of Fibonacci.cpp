...
void get(const BigInt & a){
    int j = 0;
    long long p = 0, q = 0;
    for (int i = a.digits.size() - 1; i >= 0 && j < 40; i--, j++) {
        int c = a.digits[i] + 1;
        p = (p + p_pow[j] * c) % M;
        q = (q + q_pow[j] * c) % M;
        auto it = mp.find(make_pair(p, q));
        if (it == mp.end()) {
            mp[make_pair(p, q)] = ind;
        }
    }
}
