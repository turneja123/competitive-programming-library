//https://evaluator.hsin.hr/events/coci26_2/tasks/HONI252626natjecanje/
#include <bits/stdc++.h>
...
vector<int> bfs(int stx, int sty, int n, int m, int k) {
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < m; j++) {
            dist[i][j] = INF;
        }
    ...
    while (q.size()) {
        ...
        for (int dx = -1; dx <= 1; dx++) {
            for (int dy = -1; dy <= 1; dy++) {
                if (abs(dx) + abs(dy) == 1 && x + dx >= 0 && x + dx < n && y + dy >= 0 && y + dy < m && s[x + dx][y + dy] != '#' && dist[x + dx][y + dy] > dist[x][y] + 1) {
...
int main() {
    ...
    if (k % 2 == 1) {
        ...
        coord[k] = {stx, sty};
        k++;
        nodes.push_back({stx, sty});
    }


    vector<int> d0 = bfs(stx, sty, n, m, k);
    int can = 1;
    ll ans = 0;
    for (int i = 0; i < k; i++) {
        if (d0[i] == INF) {
    ...
    for (int i = 0; i < k; i++) {
        vector<int> di = bfs(coord[i].first, coord[i].second, n, m, k);
        for (int j = i + 1; j < k; j++) {
            M.add_edge(i + 1, j + 1, d0[i] + d0[j] - di[j]);
        }
    }
    cout << ans - M.solve().first;
