//https://dmoj.ca/problem/ioi13p6io
#include <bits/stdc++.h>
...
struct TreapNode {
    ...
    TreapNode(pair<int, int> key, long long val) : key(key), priority(generator()), cnt(1), val(val), gcd(val), left(nullptr), right(nullptr) {}
};
...
long long get_gcd(TreapNode* node) {
...
void upd_cnt(TreapNode* node) {
    if (node) {
        ...
        node->gcd = __gcd(node->val, __gcd(get_gcd(node->left), get_gcd(node->right)));
    }
}
...
long long query(int node, int l, int r, int lq, int rq, int y1, int y2) {
    ...
    if (nodes[node].left != -1) {
        ans = __gcd(ans, query(nodes[node].left, l, mid, lq, rq, y1, y2));
    }
    if (nodes[node].right != -1) {
        ans = __gcd(ans, query(nodes[node].right, mid + 1, r, lq, rq, y1, y2));
    }
    return ans;
