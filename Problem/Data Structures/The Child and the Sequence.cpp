...
void apply_set(int node, int l, int r, int val) {
    segtree[node].sum = (long long)(r - l + 1) * val;
    segtree[node].mx = val;
...
void update_mod(int l, int r, int lq, int rq, int val, int node) {
    if (r < lq || l > rq || segtree[node].mx < val) {
        return;
    }
    ...
    int mid = (l + r) / 2;
    compose(node, l, r);
    update_mod(l, mid, lq, rq, val, 2 * node + 1);
    update_mod(mid + 1, r, lq, rq, val, 2 * node + 2);
    segtree[node] = combine(segtree[2 * node + 1], segtree[2 * node + 2]);
}
...
int main() {
    ...
    for (int i = 0; i < q; i++) {
        ...
        if (t == 2) {
            ...
            cin >> l >> r >> x;
            l--, r--;
            update_mod(0, n - 1, l, r, x, 0);
        } else if (t == 3) {
            int l, x;
            cin >> l >> x;
            l--;
            update_set(0, n - 1, l, l, x, 0);
        } else {
            int l, r;
            cin >> l >> r;
            l--, r--;
