//https://www.codechef.com/problems/NUMMAT
#include <bits/stdc++.h>
...
int get_ct(int l, int r, int lq, int rq, int node) {
    if (l > rq || r < lq) {
        return 0;
    }
    apply(node, l, r);
    ...
    int mid = (l + r) / 2;
    return get_ct(l, mid, lq, rq, 2 * node + 1) + get_ct(mid + 1, r, lq, rq, 2 * node + 2);
}
...
void incUpdate(int node, int l, int r, int lq, int rq) {
    ...
    for (int i = 0; i < 2; i++) {
        ...
        ct[i][node] = ct[i][2 * node + 1] + ct[i][2 * node + 2];
...
int get_zeroes(int l, int r, int n) {
    ...
    sum_nodes.clear();
    segtree_nodes.clear();
    return max(0ll, segtree_nodes[0]);
}
    ...
    while (t--) {
        ...
        for (int i = 0; i < q; i++) {
            ...
            if (t == 1) {
                ...
                cout << get_ct(0, n - 1, j, ans, 0) << endl;
